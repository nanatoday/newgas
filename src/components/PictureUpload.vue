...
const sendImageUrl = () => {
    ...
    picture.value[0] == undefined ? '' : formData.append("file", picture.value[0]);
...
<template>
    <v-responsive max-width="400">
            ...
            <v-card-text rounded="lg">
                <div class="border-opacity-25 cursor-pointer rounded-lg d-flex align-center justify-center"
                    :style="{ 'background-image': `url(${imageDataUrl})` }" style="
                          ...
                          height: 200px;
                        " @click="chooseImage1">
                        <div v-if="picture.length == 0" class="d-flex flex-column align-center ga-2">
                            <v-icon icon="mdi-file-upload-outline" color="#8A8A8A" size="35px"/>
        ...
        <v-progress-linear model-value="100" color="success" :height="6" v-if="picture.length != 0" class="mt-3"/>
    </v-responsive>
</template>
